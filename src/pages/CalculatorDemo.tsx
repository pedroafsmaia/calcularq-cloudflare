...
export default function CalculatorDemo() {
  ...
  const [profitProfile, setProfitProfile] = useState<DemoProfitProfileKey>("estabelecido");
  ...
  const results = useMemo(() => {
    ...
    return {
      globalComplexity: calc.cExp,
      adjustedHourlyRate: calc.htAj,
      projectPrice: calc.projectPrice,
  ...
  const displayValues = useMemo(() => {
    const totalVariableExpenses = variableExpenses.reduce((sum, exp) => sum + exp.value, 0);
    const adjustedHourlyRate = demoHTaj;
    ...
    const profit =
      minHourlyRate && adjustedHourlyRate > 0 && estimatedHours > 0
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-center">
          <p className="text-sm font-medium text-amber-800">
            Calcularq Demo (experimental) ·{" "}
            <a href="/calculator" className="underline">
              Acessar calculadora de produção
            </a>
          </p>
        </div>
